...
const normalizeClassNames = (className: string) => {
  const twClasses = classnames(
    "border-0 border-t-4 border-gray-200 my-10 mx-auto w-1/4",
    className
  ).split(" ");

  if (twClasses.includes("is-style-wide")) {
    return classnames(
      removeClassesFromArray(["is-style-wide", "w-1/4"], twClasses),
      "w-full"
    );
  }
  ...
  if (twClasses.includes("is-style-dots")) {
    return classnames(
      removeClassesFromArray(["border-gray-200", "is-style-dots"], twClasses),
      "border-dashed",
      "border-gray-400"
    );
  }
